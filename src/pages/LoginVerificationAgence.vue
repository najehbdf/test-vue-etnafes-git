    ...
    >
      <div class="row">
        <div class="col-md-4 offset-md-7" v-if="showLogin==true">
          ...
          >
            <div class="panel-heading">
              <h3 class="panel-title" style="color:#000">Prestataire de Services</h3>
            </div>
        ...
        <div class="col-md-4 offset-md-7" v-if="showLogin==false">
          ...
          >
            <div class="panel-heading">
              <h3 class="panel-title" style="color:#000">Vérification compte</h3>
            </div>
            <div class="panel-body">
              <form>
                <div class="form-group">
...
export default {
  ...
  methods: {
    ...
    submitPassword() {
      axios
        .post(`${apiDomain}/api/recoverNewPassword-agence`, {
        ...
        .then(res => {
          if (res.status == 200) {
            ...
            this.clear();
            //this.$router.push({ path: "/loginClient" });
          }
        })
        .catch(error => {
    ...
    sendToken() {
      ...
      if (!this.erreurEmail) {
        ...
        this.axios
          .post(`${apiDomain}/api/send-token-agence`, data)
          .then(() => {
    ...
    validateToken() {
      ...
      if (!this.errorToken) {
        ...
        axios
          .post(`${apiDomain}/api/validate-token-agence`, data)
          .then(response => {
    ...
    verifEmailClient() {
      ...
      if (!this.errorTokenVerifEmail) {
        ...
        axios
          .post(`${apiDomain}/api/validate-email-agence`, data)
          .then(response => {
    ...
    performLogin() {
      this.isLoading = true;
      this.$store
        .dispatch("performLoginAgenceAction", {
          email: this.email,
          password: this.password
        })
        .then(res => {
          this.isLoading = false;
          this.$router.push("/dashboard_agence");
        })
        .catch(error => {
          if (error.response && error.response.status == 401) {
          ...
          if (error.response && error.response.status == 422) {
            this.error = "compte en cours de traitement.";
          }
        });
    }
  }
