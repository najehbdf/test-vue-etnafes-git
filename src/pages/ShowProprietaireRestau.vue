    ...
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb" style="background-color: #a6bfe6 !important">
        <li class="breadcrumb-item">
          <router-link to="/allclients">Inscriptions</router-link>
        </li>
        <li
          ...
          aria-current="page"
        >
          Détails Proprietaire restaurant
        </li>
      </ol>
    </nav>
    <div class="row">
      <div class="col-md-6">
        <card style="border: solid 1px; padding: 15px">
          <template slot="header">
            <h5 class="title">Détails Proprietaire</h5>
          </template>
          <div class="row">
...
export default {
  ...
  created() {
    this.fetchProprietaire(this.$route.params.id);
    this.fetchAgences();
    this.fetchPaiements();
  },
  methods: {
    ...
    fetchProprietaire(id) {
      fetch(`${apiDomain}/api/proprietaire_restau/${id}`)
        ...
        .then((res) => {
          ...
          this.client = res.proprietaire.client;
